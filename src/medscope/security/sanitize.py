...
_ATTACK_PATTERNS: list[tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"</?s>|<\|.*?\|>|\{\{.*?\}\}|\[/?INST\]|```", re.I),
     "delimiter_injection"),
    # "指示"/"指引"/"说明" are at least as common as "指令" in ordinary Chinese,
    # and this project's injection surface is clinical history written by
    # clinicians — the phrasing is whatever a human happened to type, not what
    # a prompt-engineering glossary would predict.
    (re.compile(
        r"(忽略|无视|不要理会|不用管)[^。]{0,12}(之前|上述|前面|所有|以上)[^。]{0,12}"
        r"(指令|指示|指引|说明|规则|提示|要求|设定)"), "instruction_override_cn"),
    (re.compile(
